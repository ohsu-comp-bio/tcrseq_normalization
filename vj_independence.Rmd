...
```
```{r}
# Also need to make a data frame in order to do the stepwise regression.
# A few options : 260 interaction terms as columns
# This is most likely incorrect and/or irrelevant
# add.lms <- list()
# mult.lms <- list()
# mult.lms.r2 <- list()
# vs <- as.character(unique(log2.melt.count.df$V))
# js <- as.character(unique(log2.melt.count.df$J))
# for (i in 1:length(vs)){
#   v <- vs[i]
#   for(k in 1:length(js)){
#     j <- js[k]
#     curr.sub.melt <- log2.melt.count.df[!(log2.melt.count.df$V == v &
#                                             log2.melt.count.df$J == j),]
#     curr.mult <- lm(value ~ V * J, curr.sub.melt)
#     curr.r2 <- round(summary(curr.mult)$adj.r.squared, digits = 4)
#     mult.lms.r2 <- c(mult.lms.r2, curr.r2)
#   }
#   
# }
```
...
finish <- lm(value ~ V * J * combos, log2.melt.count.df)
...
test <- chisq.test(chi.sq.df, correct = F)
summary(test)