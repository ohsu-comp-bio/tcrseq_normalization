...
for (i in 1:length(align.files)){
  ...
  curr.align <- suppressWarnings(fread(paste(align.dir, align.files[i], sep = ''), na.strings = c('', ' ', NA),
                      showProgress = F))
  alignments <- c(alignments, curr.align[,.N])
...
vj.counts <- vj.counts[order(-N)]
  ...
  # Sort and extract worst 5
  #vj.counts <- vj.counts[order(N)]
  #low.vj <- vj.counts[1:5,]
  #low.vj$sample <- rep(index, 5)
  #low.vj$total.count <- rep(length(curr.align$`All V hits`), 5)
  #low.vj$proportion <- top.vj$N / top.vj$total.count
...
proportion.not.spikes <- round(despiked.counts / pear.counts * 100, digits = 2)
...
proportion.p14.of.despiked <- round(p14.contamination / despiked.counts * 100, digits = 2)
proportion.p14.of.despiked2 <- c(proportion.p14.of.despiked[1:16], NA, proportion.p14.of.despiked[17:19])

compare.unknowns <- data.frame("Prop.not.spikes" = proportion.not.spikes, "Prop.of.not.spike.p14" = proportion.p14.of.despiked2)
```
...
proportion.not.spikes.or.p14 <- round(unaccounted / pear.counts * 100, digits = 2)

prop.not.spikes.or.p14.2 <- c(proportion.not.spikes.or.p14[1:16], NA, proportion.not.spikes.or.p14[17:19])

compare.unknowns <- data.frame("Prop.not.spikes" = proportion.not.spikes, "Prop.of.not.spike.p14" = proportion.p14.of.despiked2,
                               "Prop.not.spikes.or.p14" = prop.not.spikes.or.p14.2)
compare.unknowns
cat("Proportion of Total Reads that aren't spikes (currently unaccounted for)")
summary(proportion.not.spikes)
cat("Proportion of Unnacounted Reads that are likely p14 contaminants")
summary(proportion.p14.of.despiked)
cat("Proportion of Total Unaccount Reads (Not spikes and not p14 contamination)")
...
for (i in 1:length(align.files)){
  ...
  j.counts <- curr.align[, .N, `Best J hit`]
  
  # Sort and extract top and bottom
  v.counts <- v.counts[order(-N)]
  j.counts <- j.counts[order(-N)]
  top.v <- v.counts[1:5,]
  top.j <- j.counts[1:5,]
  colnames(top.v) <- c("Best V Hit", "Best.V.Count")
  colnames(top.j) <- c("Best J Hit", "Best.J.Count")
  v.counts <- v.counts[order(N)]
  j.counts <- j.counts[order(N)]
  low.v <- v.counts[1:5,]
  low.j <- j.counts[1:5,]
  colnames(low.v) <- c("Worst V Hit", "Worst.V.Count")
  colnames(low.j) <- c("Worst J Hit", "Worst.J.Count")
  top.v$sample <- rep(index, 5)
  top.j$sample <- rep(index, 5)
  top.v$total.count <- rep(length(curr.align$`All V hits`), 5)
  top.j$total.count <- rep(length(curr.align$`All J hits`), 5)
  top.v$best.proportion <- top.v$Best.V.Count / top.v$total.count * 100
  top.j$best.proportion <- top.j$Best.J.Count / top.v$total.count * 100
  top.v$`Worst V Hit` <- low.v$`Worst V Hit`
  top.j$`Worst J Hit` <- low.j$`Worst J Hit`
  top.v$Worst.V.Count <- low.v$Worst.V.Count
  top.j$Worst.J.Count <- low.j$Worst.J.Count
  top.v$worst.proportion <- top.v$Worst.V.Count / top.v$total.count * 100
  top.j$worst.proportion <- top.j$Worst.J.Count / top.j$total.count * 100
  top.v <- top.v[,.(`sample`, `total.count`, `Best V Hit`,`Best.V.Count`, `best.proportion`, `Worst V Hit`, 
                    `Worst.V.Count`, `worst.proportion`)]
  top.j <- top.j[,.(`sample`, `total.count`, `Best J Hit`,`Best.J.Count`, `best.proportion`, `Worst J Hit`, 
                    `Worst.J.Count`, `worst.proportion`)]
  # Combine
  v.top.5 <- rbind(v.top.5, top.v)
  j.top.5 <- rbind(j.top.5, top.j)
  
} # for


summary.v.top.5 <- v.top.5[, sum(`best.proportion`), `sample`]
summary.v.low.5 <- v.top.5[, sum(`worst.proportion`), `sample`]
top.5.counts <- v.top.5[, .N, `Best V Hit`]
low.5.counts <- v.top.5[, .N, `Worst V Hit`]
...
top.5.spike.v.25bp <- NULL
top.5.spike.j.25bp <- NULL
for (i in 1:length(spike.files)){
  ...
  group.j <- curr.spike[, sum(V6), V5]
  group.j <- group.j[order(-V1)]
  low.v <- group.v[16:20,]
  low.j <- group.j[9:13,]
  colnames(low.v) <- c("Worst V Hit", "worst.v.count")
  colnames(low.v) <- c("Worst J Hit", "worst.j.count")
  group.v <- group.v[1:5,]
  group.j <- group.j[1:5,]
  colnames(group.v) <- c("Best V Hit", "best.v.count")
  colnames(group.j) <- c("Best J Hit", "best.j.count")
  group.v$sample <- rep(index, 5)
  group.j$sample <- rep(index, 5)
  group.v$total.count <- rep(sum(curr.spike$V6), 5)
  group.j$total.count <- rep(sum(curr.spike$V6), 5)
  group.v$top.proportion <- group.v$best.v.count / group.v$total.count * 100
  group.j$top.proportion <- group.j$best.j.count / group.j$total.count * 100
  group.v <- cbind(group.v, low.v)
  colnames(group.v)[c(6,7)] <- c("Worst V Hit", "worst.v.count")
  group.j <- cbind(group.j, low.j)
  colnames(group.j)[c(6,7)] <- c("Worst J Hit", "worst.j.count")
  group.v$worst.proportion <- group.v$worst.v.count / group.v$total.count * 100
  group.j$worst.proportion <- group.j$worst.j.count / group.j$total.count * 100
  group.v <- group.v[,.(`sample`, `total.count`, `Best V Hit`, `best.v.count`, `top.proportion`, `Worst V Hit`, `worst.v.count`, 
                        `worst.proportion`)]
  group.j <- group.j[,.(`sample`, `total.count`, `Best J Hit`, `best.j.count`, `top.proportion`, `Worst J Hit`, `worst.j.count`, 
                        `worst.proportion`)]
  top.5.spike.v.25bp <- rbind(top.5.spike.v.25bp, group.v)
  top.5.spike.j.25bp <- rbind(top.5.spike.j.25bp, group.j)
  
  # Again for low 5
} # for

head(top.5.spike.v.25bp)
head(top.5.spike.j.25bp)

summary.top.5.v.spike <- top.5.spike.v.25bp[, sum(`top.proportion`), `sample`]
summary.low.5.v.spike <- top.5.spike.v.25bp[, sum(`worst.proportion`), `sample`]
top.5.v.spike.counts <- top.5.spike.v.25bp[, .N, `Best V Hit`]
low.5.v.spike.counts <- top.5.spike.v.25bp[, .N, `Worst V Hit`]
cat("Top 5")
top.5.v.spike.counts
cat("Worst 5")
low.5.v.spike.counts

summary.top.5.j.spike <- top.5.spike.j.25bp[, sum(`top.proportion`), `sample`]
summary.low.5.j.spike <- top.5.spike.j.25bp[, sum(`worst.proportion`), `sample`]
top.5.j.spike.counts <- top.5.spike.j.25bp[, .N, `Best J Hit`]
low.5.j.spike.counts <- top.5.spike.j.25bp[, .N, `Worst J Hit`]
cat("Top 5")
top.5.j.spike.counts
cat("Worst 5")
low.5.j.spike.counts
```
...
```{r, echo = F}
# Let's try and see if they're primer-dimers or some chimeric read caused by mis-amplification. Process:
# 
...
new.not.spikes <- data.frame("Sample" = c(seq(1:16), seq(18:20)), 
                             "Orig.Prop.Not.Spikes" = paste(proportion.not.spikes, "%", sep = '')[c(1:16,18:20)],
                             "Orig.Unaccounted" = paste(proportion.not.spikes.or.p14, "%", sep = ''),
                             "Prop.Not.Spikes" = paste(new.proportion.not.spikes, "%", sep = ''),
                             "Prop.Unaccounted" = paste(new.proportion.not.spikes.or.p14, "%", sep = ''))
new.not.spikes
```
...
```{r, echo = F}
blast.dir <- "/Volumes/DNA160609LC/blast/unaccounted/output/"
blast.files <- list.files(blast.dir)
blast.files <- blast.files[order(as.numeric(gsub(".*_S|_blast.*", '', blast.files)))]
fasta.dir <- "/Volumes/DNA160609LC/blast/unaccounted/fastas/"
fasta.files <- list.files(fasta.dir)
fasta.files <- fasta.files[order(as.numeric(gsub(".*_S|_unacc.*", '', fasta.files)))]

# First compare reads
fasta.reads <- NULL
blast.reads <- NULL
for (i in 1:length(fasta.files)){
  curr.fasta <- readDNAStringSet(paste(fasta.dir, fasta.files[i], sep = ''))
  curr.blast <- fread(paste(blast.dir, blast.files[i], sep = ''))
  
  fasta.reads <- c(fasta.reads, length(curr.fasta))
  blast.reads <- c(blast.reads, curr.blast[,.N])
} # for

new.alignments <- c(alignments[1:16], NA, alignments[17:19])

blast.compare <- data.frame("Fasta.reads" = fasta.reads, "Blast.reads" = blast.reads)
blast.compare
```


```{r, echo = F}
#blast.results.dir <- "/Volumes/DNA160609LC/blast/spikes/primer_check_output/primer_results/"
#blast.results.files <- list.files(blast.results.dir)
#blast.results.files <- blast.results.files[order(as.numeric(gsub(".*_S|_blast.*", '', blast.results.files)))]

#for (i in 1:length(blast.results.files)){
#  curr.blast.results <- fread(paste(blast.results.dir, blast.results.files[i], sep = ''))
#  curr.sum <- curr.blast.results[,sum(V13)]
